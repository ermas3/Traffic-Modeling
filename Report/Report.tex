...
was set to $p=0.2$. Note that these simulation parameters remained unchanged during all following simulations. Fundamental diagrams were generated
by increasing the number of cars $n_{cars}$ so that the car density varied between 0 and 1. 
...
as if the system becomes independant of it size already at approximately $L=60$. When using 2 and 3 lanes in figure \ref*{finite size effects} b) and c) a
larger value of $L$ appears to be necessary in order to 
